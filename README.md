...
## DOCX exports (Milestone 3A)

Once a `ContractVersion` is final-approved, the **Exports** page offers two real DOCX downloads:

- **Download external clean DOCX** — counterparty-facing contract. Includes project name, contract version, body, source pack id, playbook id, and a simple signature block. Generated by `packages/core/src/export-renderer/build-clean.ts`. The renderer self-scrubs against an internal-commentary marker list before emitting and refuses to render if any marker is present (PLATFORM_BRIEF.md §5 rules 6/7).
- **Download internal commentary DOCX** — internal legal commentary. Carries the full contract body PLUS an `INTERNAL ONLY — 내부 법무 검토 전용` banner, the Issue Card decision trail (including rejected cards), deterministic-QA summary, and AgentRun / provider summary. Generated by `packages/core/src/export-renderer/build-commentary.ts`.

How the binary flows:

1. The browser POSTs the current `ProjectState` and the chosen `export_type` to **`POST /api/exports/docx`** (`packages/web/app/api/exports/docx/route.ts`).
2. The server route re-validates the final-approval guard, invokes the renderer, and streams back the `.docx` bytes with `Content-Type: application/vnd.openxmlformats-officedocument.wordprocessingml.document` and a `Content-Disposition: attachment` header.
3. The browser turns the response into a `Blob`, triggers a download, and records an `ExportFile` + `AuditLog` entry locally (with file name + source pack id + playbook id metadata). **Binary bytes are never stored in `ProjectState` or `localStorage`.**
...
- The clean DOCX MUST NOT contain `법무주석`, `[COMMENTARY]`, `[INTERNAL]`, `[REDLINE_RATIONALE]`, `[NEGOTIATION_GUIDANCE]`, or `internal legal commentary` — asserted by `packages/core/tests/export-renderer.test.ts` and by `packages/web/e2e/docx-export.spec.ts` (which unzips the downloaded file and greps the XML).
- The commentary DOCX self-identifies with the `INTERNAL ONLY — 내부 법무 검토 전용` banner and footer; an asserted test checks both.
- Rejected Issue Cards never appear in the clean DOCX — both because the Revision Agent never applies them (PLATFORM_BRIEF.md §5 rule 5) and because the clean renderer emits no Issue Card text at all.
- Exports are generated **in memory only**. Nothing is written to disk on the server. Nothing is sent externally — PLATFORM_BRIEF.md §5 rule 2 still applies.
...
npm install
npm run dev -w @contractops/web    # http://localhost:3000
# Walk the mock workflow up to final approval, then click the buttons on
# /projects/<id>/exports — your browser will download <project>_v<n>_clean.docx
# and <project>_v<n>_commentary_INTERNAL.docx.
```

`*.docx` is gitignored and `npm run repo:hygiene` refuses to allow a tracked `.docx` artifact. Generated exports from local runs land in your Downloads folder; never commit them.
